...
class Finder
{
    ...
    /**
     * Search for the given package on provider(s). If a provider URL is given within the option, the search will occur
     * only against that provider
     *
     * @param string $packageName
     * @param array $options
     *
    ...
    public function searchProviders($packageName, array $options = array())
    {
        if (isset($options['provider'])) {
            $cacheKey = Provider::getCacheKey($options);
            ...
            return $this->providers[$cacheKey]->search($packageName, $options);
        } else {
    ...
    protected function loadAvailableProviders()
    {
        ...
        foreach ($collection as $data) {
            ...
            if ($provider->fromProvider($data)) {
                ...
                $this->providers[$provider->getCache()] = $provider;
            }
        }
    }
